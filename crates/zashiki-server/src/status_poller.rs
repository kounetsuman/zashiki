...
use zashiki_core::session_state::{
    apply_jsonl_fallback, apply_startup_grace, count_running_subagents, detect_state, has_bg_agent,
    hook_event_fresh_within_sec, is_limit_reached, is_menu_open, open_tasks_remaining,
    resolve_state, skill_agents_running, startup_grace_polls, subagent_fresh_within_sec,
    CockpitTerminalState, DetectStateOptions,
};
    ...
    async fn evaluate_window<P: PollerPorts>(
        ...
        if bg_present {
            running_subagents = if let Some(n) = skill_agents {
                ...
                count_running_subagents(&ages, subagent_fresh_within_sec(config.poll_sec)).max(1)
            } else {
                1
            };
        }
