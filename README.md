...
Grab the macOS `Zashiki_*.dmg` from [Releases](https://github.com/kounetsuman/zashiki/releases), mount it, and drag `Zashiki.app` into `/Applications`. The `.dmg` is code-signed with a Developer ID certificate and notarized, so it opens without a Gatekeeper prompt. Pushing a `vX.Y.Z` tag triggers [`.github/workflows/release.yml`](.github/workflows/release.yml), which builds a self-contained bundle and publishes it as a draft Release. The one-line `curl` installer in [Quick Start](#quick-start) does the same.
