...
- **Pinned tab** — any tab can be pinned from its right-click menu. Pinned tabs render in their own fixed strip at the left, so scrolling the other tabs never hides them. The strip is capped at two thirds of the bar — or one tab's width, whichever is wider — and scrolls within itself past that, so pinning more tabs than fit cannot swallow the whole bar. The Memo tab is pinned implicitly (front-most, no pin toggle). The canonical rules live in `src/tabs/tab-model.test.ts`.
