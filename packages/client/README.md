...
└ Cockpit View — shows a Cockpit Terminal | a Viewer (one per Cockpit Tab)
     └ Cockpit Terminal — the durable unit; a Claude Session (sid) runs inside it
          ...
          ├ Background Activity — runningSubagents / shellsRunning / vitestRunning / limited (orthogonal flags)
...
- **Pinned tab** — any tab can be pinned from its right-click menu. Pinned tabs render in a strip at the left that the remaining tabs never scroll, so they stay visible while those scroll horizontally. The strip is capped at two thirds of the bar and scrolls within itself past that, so pinning more tabs than fit cannot swallow the whole bar. The Memo tab is pinned implicitly (front-most, no pin toggle). The canonical rules live in `src/tabs/tab-model.test.ts`.
