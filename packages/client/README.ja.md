...
- **Pinned tab（ピン留めタブ）** — 各タブは右クリックメニューからピン留めできる。ピン留めしたタブは左端の専用の固定ストリップに並び、残りのタブを横スクロールしても隠れない。このストリップの上限はタブバーの 2/3（ただしタブ1枚分を下回らない）で、それを超える分はストリップ内で独立にスクロールする（ピン留めしすぎてもタブバー全体を占有しない）。Memo タブは暗黙的にピン留めされる（先頭固定・ピン留めトグルなし）。仕様の正本は `src/tabs/tab-model.test.ts`。
...
- [ ] ホイールで xterm.js 自身の scrollback を遡れ、最初のプロンプトまで戻れる
- [ ] 最下部までスクロールすると live 表示へ戻る
- [ ] スクロールで遡っている間もキー入力（q 等）がターミナルへ届く
- [ ] 大量出力（`yes` 等）中でも UI が固まらない（フロー制御。止めた後に追いつく）

### コピー

- [ ] Shift+ドラッグで xterm.js のテキスト選択ができる（アプリ側の mouse mode と衝突しない）
