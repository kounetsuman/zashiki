...
- **Pinned tab（ピン留めタブ）** — 各タブは右クリックメニューからピン留めできる。ピン留めしたタブは左端の固定ストリップに並び、残りのタブを横スクロールしても隠れない。このストリップはタブバーの 2/3 が上限で、それを超える分はストリップ内で独立にスクロールする（ピン留めしすぎてもタブバー全体を占有しない）。Memo タブは暗黙的にピン留めされる（先頭固定・ピン留めトグルなし）。仕様の正本は `src/tabs/tab-model.test.ts`。
