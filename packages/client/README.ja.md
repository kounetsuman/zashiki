...
- **Pinned tab（ピン留めタブ）** — 各タブは右クリックメニューからピン留めできる。ピン留めしたタブは左端の専用の固定ストリップに並び、残りのタブを横スクロールしても隠れない。このストリップの上限はタブバーの 2/3 で、収まらない分はピン留めタブ側が細くなって収まる（ピン留めしすぎてもタブバー全体を占有しない）。Memo タブは暗黙的にピン留めされる（先頭固定・ピン留めトグルなし）。仕様の正本は `src/tabs/tab-model.test.ts`。
