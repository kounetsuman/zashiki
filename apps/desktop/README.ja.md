...
macOS では ⌘Z / ⇧⌘Z を WebView より先にメニューが受け取り、既定項目が実行するネイティブの
undo はページ内エディタ自身の履歴を知らない。そのためメモとクリップボード編集では undo が
効かなかった。シェルはこの2項目を差し替え（`src-tauri/src/menu.rs`）、コマンドをページへ転送する。
ページ側の `useEditMenuHistory` が、カーソルのある対象（属する CodeMirror エディタ、または
通常のテキスト入力ならブラウザ自身の履歴）へ適用する。残りの項目は既定のままで、ネイティブ
の動作で正しく機能する。これは macOS 限定で、他プラットフォームでは Tauri もシェルもメニューを
用意しないため、そもそもキー入力が横取りされない。
