...
実ユーザーデータ（`~/.zashiki` / `~/.claude`）には一切触れない（temp は終了時に削除）。専用の
ポート 8791 で動くため、8790 で動く本番アプリと並走できる（別の demo が既に 8791 を使っている時だけ起動を拒否する）。
...
- **`tauri dev` の既定は `ZK_PORT=8790`**: `beforeDevCommand` がポートからクライアント URL を
  ...
  server とクライアントが一緒に移動する（`dev:demo` が 8790 の本番と並走して 8791 で動く仕組み）。
